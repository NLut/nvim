...
toggleterm.setup {
  size = 15,
  open_mapping = [[<C-t>]], --keybind to openterminal
  hide_numbers = true,
  shade_filetypes = {},
  ...
  persist_size = true,
  direction = "horizontal", -- float, horizontal, vertical for optional
  close_on_exit = true,
  shell = vim.o.shell,
  float_opts = {
    border = "curved",
...
function _LAZYGIT_TOGGLE() -- :lua _LAZYGIT_TOGGLE to toggle lazygit.
  lazygit:toggle()
end

local node = Terminal:new { cmd = "node", hidden = true }
function _NODE_TOGGLE() -- :lua _NODE_TOGGLE to toggle node.
  node:toggle()
end

local ncdu = Terminal:new { cmd = "ncdu", hidden = true } -- :lua _NCDU_TOGGLE to toggle ncdu which to specify file size in this dir.
function _NCDU_TOGGLE()
  ncdu:toggle()
end

local htop = Terminal:new { cmd = "htop", hidden = true } -- :lua _HTOP_TOGGLE to toggle htop which show about ram.
function _HTOP_TOGGLE()
  htop:toggle()
end

local python = Terminal:new { cmd = "python", hidden = true } -- :lua _PYTHON_TOGGLE to toggle python termianl
